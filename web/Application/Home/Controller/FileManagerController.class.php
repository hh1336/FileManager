        ...
        if ($us->hasPermission(FIdConst::WJGL_YL_FILE)) {
            ...
            if ($params["file_id"]) {
                ...
                if (in_array(strtolower($data["file_suffix"]), $officeType) || $data["file_suffix"] == "pdf") {
                    ...
                    readfile($data["file_path"] . $data["file_version"] . "." . $data["file_suffix"], "预览." . strtolower($data["file_suffix"]));
                    //$this->show(file_get_contents($data["file_path"] . $data["file_version"] . "." . $data["file_suffix"]), "utf-8", ("image/" . strtolower($data["file_suffix"])));
                } else {
